...
public class SubscriptionServiceImpl implements SubscriptionService {
    ...
    public SubscriptionDto subscribe(Long userId, SubscriptionRequestDto requestSubscription) {
        User follower = findUser(userId);
        User owner = findUser(requestSubscription.getOwnerId());

        validateSubscription(follower, owner);

        Subscription subscription = createOrUpdateSubscription(follower, owner);
    ...
    @Transactional(readOnly = true)
    @Override
    public List<SubscriberDto> getAllSubscribers(Long userId, int from, int size) {
    ...
    private Subscription createOrUpdateSubscription(User follower, User owner) {
        Subscription subscription = subscriptionMapper.toSubscription(follower, owner, LocalDateTime.now(), null, FriendshipsStatus.NO_FRIENDSHIP);
